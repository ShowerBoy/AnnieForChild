...
public class CircleProgressBar extends View {
     ...
     * 渐变圆周颜色数组
     */
    private int[] colorArray = new int[]{Color.parseColor("#FF8A0D"), Color.parseColor("#FF8A0D")};

    /**
     * 背景图片的宽高(因为是正方形，所以不需要height)
     */
    ...
    public CircleProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //
//        bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.icon_speak_big);
...
//        drawText(canvas, center, radius);
    ...
    private void drawCircle(Canvas canvas, int center, int radius) {
        ...
        circlePaint.setShadowLayer(1, 1, 1, Color.WHITE);
...
//        canvas.drawText(percent, center, baseline, textPaint); // 绘制表示进度的文字
        ...
        canvas.drawBitmap(bitmap, center - radius, center - radius, textPaint);
    }
    ...
    public void setProgress(float progress) {
        float percent = progress * maxValue / 100;
        if (percent < 0) {
            percent = 0;
        }
        if (percent > 100) {
