...
public class ControlView extends RelativeLayout implements ViewAction, ITheme {
    ...
    private List<VideoDefiniList> videoList;
    //播放的进度
    ...
    private void setViewListener() {
        ...
        mTitleDownload.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (mOnScreenClickListener != null) {
                    mOnScreenClickListener.showScreen();
                }
            }
        });
        //控制栏的播放按钮监听
        ...
        mLargeChangeQualityBtn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                Log.e("清晰度00","是否点击");
                //点击切换分辨率 显示分辨率的对话框
                if (mOnQualityBtnClickListener != null && videoList != null) {
//                    for (VideoDefiniList trackInfo : trackInfos) {
//                        //清晰度
//                        if (VideoDefiniList.getType() == TrackInfo.Type.TYPE_VOD) {
//                            qualityTrackInfos.add(trackInfo);
//                        }
//                    }
                    Log.e("清晰度3",mCurrentQuality);

                    mOnQualityBtnClickListener.onQualityBtnClick(v, videoList, mCurrentQuality);
                    mOnQualityBtnClickListener.refreshAdapter();
                }
            }
        });
...
//            mTitleDownload.setVisibility(VISIBLE);
     ...
     * @param currentQuality  当前清晰度
     */
    public void setMediaInfo(MediaInfo aliyunMediaInfo,List<VideoDefiniList> videoList1, String currentQuality) {
...
//    }
    ...
    private void updateChangeQualityBtn() {
        if (mLargeChangeQualityBtn != null) {
            Log.e("清晰度1",mCurrentQuality);
//            mLargeChangeQualityBtn.setText(QualityItem.getItem(getContext(), mCurrentQuality, isMtsSource).getName());
//            if(videoList!=null && videoList.size()>0){
            if(mCurrentQuality.length()<1){
                if(videoList!=null && videoList.size()>0){
                    mLargeChangeQualityBtn.setText(videoList.get(0).getTitle());
                }
            }else{
                Log.e("清晰度11",mCurrentQuality);
                mLargeChangeQualityBtn.setText(mCurrentQuality);
            }
...
//            }
    ...
    private void updateTitleView() {
        if (videoList != null && videoList.get(0).getTitle() != null && !("null".equals(videoList.get(0).getTitle()))) {
//            mTitlebarText.setText(videoList.get(0).getTitle());
...
//            mScreenShot.setVisibility(GONE);
    ...
    public interface OnQualityBtnClickListener {
         ...
         * @param currentQuality 当前清晰度
         */
        void onQualityBtnClick(View v, List<VideoDefiniList> qualities, String currentQuality);
    ...
    public void setOnScreenClickListener(
