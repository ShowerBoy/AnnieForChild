...
public class AliyunVodPlayerView extends RelativeLayout implements ITheme {
    ...
    // 点击投屏
    private ControlView.OnScreenClickListener mOnScreenClickListener;
    //播放按钮点击监听
     ...
     * @param theme 支持的主题
     */
    public void setVideoList(List<VideoDefiniList> list){
        videoList=list;
        String type="";
...
//                mQualityView.setQuality(videoList,type);
            }
...
//        mControlView.hide(ControlView.HideType.Normal);
    ...
    private void initControlView() {
        ...
        //改为投屏入口
        mControlView.setOnScreenClickListener(new ControlView.OnScreenClickListener() {
            @Override
        ...
        mControlView.setOnQualityBtnClickListener(new ControlView.OnQualityBtnClickListener() {
            ...
            @Override
            public void onQualityBtnClick(View v, List<VideoDefiniList> qualities, String currentQuality) {
                //显示清晰度列表
            ...
            public void refreshAdapter() {
                mQualityView.refreshCurrentQuality();
            }
        });
        //点击锁屏的按钮
    ...
    private void showThumbnailView() {
        if (mThumbnailView != null && mThumbnailPrepareSuccess) {
            ...
            if (thumbnailImageView != null) {
                ...
                layoutParams.height = layoutParams.width / 2 - px2dip(getContext(), 10);
    ...
    private void initQualityView() {
        ...
        mQualityView.setOnQualityClickListener(new QualityView.OnQualityClickListener() {
            @Override
            public void onQualityClick(VideoDefiniList qualityTrackInfo) {
                //进行清晰度的切换
//                mAliyunVodPlayer.selectTrack(qualityTrackInfo.getUrl());
                Log.e("测试清晰度",qualityTrackInfo.getTitle()+"");
...
//    }
    ...
    public void showErrorTipView(int errorCode, String errorEvent, String errorMsg) {
        ...
        if (mTipsView != null) {
            ...
            mTipsView.showErrorTipView(errorCode, "", "加载失败，点击重新加载");
        }
    }
...
//            return mAliyunVodPlayer.getAllDebugInfo();
    ...
    public void setOnScreenClickListener(
            ControlView.OnScreenClickListener listener) {
    ...
    private void sourceVideoPlayerPrepared() {
        ...
        if(videoList!=null && videoList.size()>0){
            mControlView.setMediaInfo(mAliyunMediaInfo,videoList,"");
        }else{
            mControlView.setMediaInfo(mAliyunMediaInfo,videoList,"");
        }
    ...
    private void sourceVideoPlayerError(ErrorInfo errorInfo) {
        ...
        showErrorTipView(errorInfo.getCode().getValue(), "","加载失败，点击重新加载");
